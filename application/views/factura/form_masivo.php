        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <Div class="row">
                    ...
                    <div class="col-sm-6">
                    <input type="text" class="form-control" placeholder="INGRESE LA DIRECCIÓN DEL CLIENTE" name="dire" id="dire"
                            onchange="buscar_clientes(this)">
                    </div>
...
function buscar_clientes(valor) {
    ...
    $.ajax({
        ...
        url: base_url + "factura/buscar_cliente_2/" + valor.value,
        type: 'JSON',
        dataType: 'JSON',
        success: function(dt) {
...
function traer_cliente(valor) {
    ...
    $.ajax({
        beforeSend: function() {
            if ($('#identificacion').val().length == 0 && $('#dire').val().length == 0 ) {
                swal("", "¡Ingrese un  dato. Revisar por favor!", "info");
        ...
        success: function(dt) {
            if (dt != "") {
                ...
                if (dt.cli_id ==null || dt.cli_calle_prin == null || dt.cli_email == null || dt.cli_canton == null ||  dt.cli_pais== null ||  dt.cli_telefono == null )  {
                    swal("", "¡Cliente con información incompleta. Revisar por favor!", "info");
                }else{
                agregar(dt.cli_id, dt.cli_ced_ruc, dt.cli_raz_social, dt.cli_calle_prin, dt.cli_email, dt.cli_canton, dt.cli_pais, dt.cli_telefono);
                }
...
function traer_forma(obj) {
    ...
    var my = document.querySelector("#ajuste");
    var j = (my.rows.length) - 1;
    ...
    $.ajax({
        ...
        success: function(dt) {
            ...
            if (dt != "") {
                ...
                if (j == 0) {
                        swal("Error!", "Debe agregar al menos un cliente.!", "error");
...
function save() {
    ...
    while (r < j) {
            r++;
            if ($('#identificacion' + r).html() != null) {
                if ($('#identificacion' + r).val().length == 0) {
                    $('#identificacion' + r).css({
                        borderColor: "red"
                    });
                    $('#identificacion' + r).focus();
                    return false;
                } else if ($('#nombre' + r).val().length == 0 || parseFloat($('#nombre' + r).val()) == 0) {
                    $('#nombre' + r).css({
                        borderColor: "red"
                    });
                    $('#nombre' + r).focus();
                    return false;
                } else if ($('#direccion_cliente' + r).val().length == 0) {
                    $('#direccion_cliente' + r).css({
                        borderColor: "red"
                    });
                    $('#direccion_cliente' + r).focus();
                    return false;
                } else if ($('#email_cliente' + r).val().length == 0 || parseFloat($('#email_cliente' + r).val()) == 0) {
                    $('#email_cliente' + r).css({
                        borderColor: "red"
                    });
                    $('#email_cliente' + r).focus();
                    return false;
                }
