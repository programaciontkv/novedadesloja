...
public function buscar_cliente_2($txt){
	...
	if(!empty($cns)){
		...
		foreach ($cns as $rst) {
			$n++;
			$nm = $rst->cli_raz_social;
			$lista.="<tr ><td><input type='checkbox' class='chk'  value='&#8730;' onclick=" . "traer_cliente('$rst->cli_ced_ruc')" . "  /></td><td>$rst->cli_ced_ruc</td><td>$nm</td></tr>";
		}
	$data=array(
				'lista'=>$lista,
				);
    ...
    public function show_pdf($id,$opc_id,$correo,$etiqueta){
    		...
    		$this->html2pdf->html(utf8_decode($this->load->view('pdf/'.$doc, $data, true)));
    ...
    public function show_talon($id,$opc_id,$correo,$etiqueta){
			...
			$this->html2pdf->paper(array(0,0,205.75,723.66),'portrait');
    ...
    public function set_barcode($code)
	{
		...
		$imageResource = Zend_Barcode::factory('code128', 'image', array('text'=>$code,'drawText'=>false), array())->draw();
    ...
    function envio_mail($datos){
        ...
        $this->email->subject("Cliente : $ncliente ");
		...
		$clie = ucfirst(strtolower($datos->cliente));
		$men = ucfirst(strtolower($mensaje));
        //$mensaje = $cred[7];

         // <tr class='mensaje'><td class='mensaje' > <p class='mensaje'>$men </p>  </td></tr>
        ...
        $datos_sms = "<html>
             ...
             <body>
               <table width='100%'>
                  ...
                  <tr><td></br></br> </td></tr>
                  <tr style='with:60px' ><td>Hola $clie , </td></tr>
                 ...
                 <tr>
                  ...
                  <tr>   
                       <td style='font-size:12px'>
                        ...
                        <img src='$img_mail' width='20px'><a href=''>tikvas.com</a> 
                        <img src='$img_whatsapp' width='20px'> +593 999404989 / +593 991815559
                        
                      </td>
                  </tr>
                  <tr><td style='font-size:10px'>Copyright &copy; 2022 Todos los derechos reservados <a href=''>TIKVASYST S.A.S</a></td></tr>
               </table>
             </body>
           </html>";
